...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Tag Management Button Handler
    const tagManagementBtn = document.getElementById('tagManagementBtn');
    if (tagManagementBtn) {
        tagManagementBtn.addEventListener('click', () => {
            if (selectedCourseId) {
                // Open tag management page in new tab with current course ID
                window.open(`/tag-management.html?courseId=${encodeURIComponent(selectedCourseId)}`, '_blank');
            } else {
                showNotification('Please select a course first before managing tags.', 'error');
            }
        });
    }
});
