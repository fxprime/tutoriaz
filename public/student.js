...
function initializeSocket() {
    ...
    socket.on('show_next_quiz', (data) => {
        ...
        // Validate quiz hasn't already expired
        if (data.remaining_seconds !== undefined && data.remaining_seconds <= 0) {
            console.log('⚠️ Rejecting expired quiz:', data.push_id, 'remaining:', data.remaining_seconds);
            showNotification('Quiz has already expired', 'error');
            return;
        }
    ...
    socket.on('quiz_undo', (data) => {
        ...
        const pushIdMatches = undoPushId && currentQuiz && currentQuiz.push_id === undoPushId;
        const quizIdMatches = undoQuizId && currentQuiz && currentQuiz.quiz_id === undoQuizId;

        console.log('Checking undo match:');
        console.log('  - undoPushId:', undoPushId);
        console.log('  - currentQuiz pushId:', currentQuiz ? currentQuiz.push_id : 'none');
        console.log('  - undoQuizId:', undoQuizId);
        console.log('  - currentQuiz quizId:', currentQuiz ? currentQuiz.quiz_id : 'none');
        console.log('  - pushIdMatches:', pushIdMatches);
        console.log('  - quizIdMatches:', quizIdMatches);

        if (pushIdMatches || quizIdMatches) {
            console.log('✓ Match! Hiding quiz with quiz_id:', undoQuizId || (currentQuiz ? currentQuiz.quiz_id : 'none'));
            try {
                hideQuiz();
            ...
            console.log('  - Not hiding overlay because undo doesn\'t match current quiz');
...
function showQuiz(quizData) {
    ...
    // Final validation: Don't show quiz if it has already expired
    if (remaining <= 0) {
        console.log('⚠️ Quiz already expired in showQuiz():', quizData.push_id, 'remaining:', remaining);
        showNotification('This quiz has already expired', 'error');

        // Request next quiz from server
        if (socket && socket.connected) {
            setTimeout(() => {
                socket.emit('get_my_queue');
            }, 1000);
...
// ===========================
// LISTEN FOR QUIZ TRIGGERS FROM PROGRESS TRACKER (IFRAME)
// ===========================

// Listen for postMessage from iframe (progress-tracker.js)
window.addEventListener('message', function (event) {
    // Security check - only accept from same origin or our server
    if (event.origin !== window.location.origin && !event.origin.startsWith('http://127.0.0.1') && !event.origin.startsWith('http://localhost')) {
        return;
    }

    const data = event.data;

    if (data && data.type === 'TRIGGER_QUIZ' && data.quizData) {
        console.log('[Student] Received TRIGGER_QUIZ postMessage from iframe:', data);
        console.log('[Student] Quiz data:', data.quizData);

        const quizData = data.quizData;

        // Server returns quiz data in format: { quizId, title, questionType, timeoutSeconds, ... }
        if (quizData && quizData.quizId) {
            const pushId = `auto-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

            // Register the quiz push with the server first
            fetch('/api/auto-trigger-quiz', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
                ...
                .then(result => {
                    if (result.success && result.quiz) {
                        // Create quiz push object with server-validated data
                        const quizPush = {
                            push_id: pushId,
                            quiz_id: result.quiz.id,
                            quiz: result.quiz,
                            course_id: data.courseId || selectedCourseId,
                            timeout_seconds: result.quiz.timeout_seconds || 60,
                            remaining_seconds: result.quiz.timeout_seconds || 60,
                            triggered_by: 'progress-tracker',
                            section_id: data.sectionId
                        };

                        console.log('[Student] Quiz registered, showing quiz:', quizPush);
                        showQuiz(quizPush);
                    } else {
                        console.error('[Student] Failed to register quiz:', result);
                        showNotification('Failed to load quiz', 'error');
                    }
                })
                .catch(error => {
                    console.error('[Student] Error registering quiz:', error);
                    showNotification('Failed to load quiz', 'error');
                });
        } else {
            console.error('[Student] Invalid quiz data format - missing quizId:', quizData);
        }
    }
});

// Listen for custom event (alternative method)
document.addEventListener('progressQuizTrigger', function (event) {
    console.log('[Student] Received progressQuizTrigger custom event:', event.detail);

    if (event.detail && event.detail.quizData) {
        const data = event.detail;
        const quizData = data.quizData;

        if (quizData && quizData.quizId) {
            const quizPush = {
                push_id: `auto-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                quiz_id: quizData.quizId,
                quiz: {
                    id: quizData.quizId,
                    title: quizData.title || 'Quiz',
                    content_text: quizData.contentText || quizData.content_text || '',
                    question_type: quizData.questionType || quizData.question_type || 'text',
                    timeout_seconds: quizData.timeoutSeconds || quizData.timeout_seconds || 60,
                    options: quizData.options || [],
                    correct_answer: quizData.correctAnswer || quizData.correct_answer || null,
                    is_scored: quizData.isScored !== undefined ? quizData.isScored : (quizData.is_scored !== undefined ? quizData.is_scored : true),
                    points: quizData.points || 1
                },
                course_id: data.courseId || selectedCourseId,
                timeout_seconds: quizData.timeoutSeconds || quizData.timeout_seconds || 60,
                remaining_seconds: quizData.timeoutSeconds || quizData.timeout_seconds || 60,
                triggered_by: 'progress-tracker',
                section_id: data.sectionId
            };

            console.log('[Student] Showing auto-triggered quiz from custom event:', quizPush);
            showQuiz(quizPush);
        }
    }
});

console.log('[Student] Quiz trigger event listeners initialized');
